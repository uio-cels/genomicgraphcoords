...
def make_graph(seq1, seq2):
    ...
    for c, c2 in zip(seq1+"$", seq2+"#"):
        ...
        if cur_mode == "poly":
            cur_block_id = "main"+str(cur_len)+cur_mode+str(main_block_id)
            main_block_ids.append(cur_block_id)
            blocks[cur_block_id] = RegionPath(
                cur_block_id, {"main": lin_refs["main"]})
            main_block_id += 1
            cur_block_id = "alt"+str(cur_len)+cur_mode +str(alt_block_id)
            alt_block_ids.append(cur_block_id)
            blocks[cur_block_id] = RegionPath(
                cur_block_id, {"alt": lin_refs["alt"]})
            alt_block_id += 1
        else:
            if "main" in lin_refs:
                this_block_id = "main" + str(cur_len)+ cur_mode +  str(main_block_id)
                main_block_ids.append(this_block_id)
                main_block_id += 1
            else:
                this_block_id = "alt" + str(cur_len) + cur_mode + str(alt_block_id)
                alt_block_id += 1
            if "alt" in lin_refs:
        ...
        cur_mode = new_mode
        cur_len = 1
    print(alt_block_ids[-2:])
    print(main_block_ids[-2:])
...
def parse_blast(filename):
    ...
    q_seq = "".join(q_seqs)
    s_seq = "".join(s_seqs)
    print(len(q_seq.replace("-","")))
    print(len(s_seq.replace("-","")))
    print(s_seq[-10:])
    print(q_seq[-10:])
    return make_graph(s_seq, q_seq)

if __name__ == "__main__":
    graph = parse_blast("chr13alt_align.txt")
    graph_interval = GraphInterval.linear_segment_to_graph(
        graph, "main", 52068, 53115)
    print(graph_interval)
    graph_interval = GraphInterval.linear_segment_to_graph(
        graph, "alt", 35918, 36958)
    print(graph_interval)
