...
class RegionPath(object):
    ...
    def get_length(self):
        return max([lr.end-lr.start for lr in list(self.linear_references.values())])

    def contains(self, lin_ref):
        return any([my_ref.contains(lin_ref) for my_ref
                    in list(self.linear_references.values())])
    ...
    def length(self):
        return max([lr.length() for lr in list(self.linear_references.values())])
    ...
    def __str__(self):
        return " RegionPaths: " + ";".join([str(lin_ref) for lin_ref
                                            in list(self.linear_references.items())])
